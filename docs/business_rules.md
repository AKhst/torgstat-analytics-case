...
- Версия: `0.6`
- Статус: смешанный — core entity и financial timeline rules `APPROVED`,
  invoice currency fallback и FX-date policy `DRAFT`
- Реализация: core rules внедрены; fallback пустой invoice currency на workspace
  currency не реализован, поэтому текущая модель сохраняет quality flag и
  исключает строку из certified USD amount
...
- Причина ограничения: это даёт один возможный input для восстановления
  пропущенной invoice currency без исторической таблицы изменений; само
  применение fallback требует отдельного утверждённого правила.
...
- Целевое поведение после утверждения: большинство записей восстанавливаются
  через workspace default; текущая реализация их не восстанавливает.
...
Отдельно необходимо согласовать:

- invoice currency fallback и допустимые доказательства для его применения;
- дату FX-курса для финансовой конвертации;
- as-of определение active paid workspace.
